...
def vizualize_flow(img, flo, save, counter, display_scale):
    ...
    cv2.imshow("Optical Flow", display_img / 255.0)
...
def inference(args):
    ...
    with torch.no_grad():
        while True:
            ...
            ret = vizualize_flow(
                frame_1, flow_up, save, counter, args.display_scale
            )
            if not ret:
                break
            frame_1 = frame_2
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="scale factor for visualization window (e.g. 0.6)",
    )
